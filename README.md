...
このリポジトリのオリジナルのスクリプト、文書、パッチメタデータは GNU GPL version 3
or later（GPL-3.0-or-later）でライセンスされています。全文は
`[LICENSE](LICENSE)` を参照してください。

パッチに含まれるZed由来のコードは、変更対象となるupstreamファイルの著作権表示と
ライセンスに従います。対象Zedおよび依存コンポーネントのライセンス・配布条件を
維持してください。帰属情報は `[NOTICE](NOTICE)` に記載しています。
これはZed Industriesによる公式プロジェクトではありません。
